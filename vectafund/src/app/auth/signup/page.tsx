...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Check your email for a magic link to complete signup!',
      })
    } catch (error: any) {
      setMessage({
        type: 'error',
    ...
    <div className="min-h-screen bg-vf-black flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-vf-dark to-vf-black p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-8">
          <h1 className="font-display text-4xl font-bold text-vf-text">
            Join the Alpha
          </h1>
          
          <div className="space-y-4">
            {[
              'Track 30+ top crypto VCs',
              'Real-time Email & Telegram alerts',
              'Ethos reputation scores for trust',
              'Weekly digest of top deals',
              '100% free during beta',
            ].map((feature, i) => (
              <div key={i} className="flex items-center gap-3">
                <CheckCircle2 className="w-5 h-5 text-vf-accent shrink-0" />
                <span className="text-vf-muted">{feature}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <p className="text-vf-muted mb-8">
            Start tracking smart money in crypto today
          </p>
          ...
          <p className="mt-6 text-center text-vf-muted text-sm">
            By signing up, you agree to our Terms of Service and Privacy Policy
          </p>

          <p className="mt-4 text-center text-vf-muted">
            Already have an account?{' '}
            <Link href="/auth/login" className="text-vf-accent hover:underline">
              Sign in
            </Link>
          </p>
        </div>
